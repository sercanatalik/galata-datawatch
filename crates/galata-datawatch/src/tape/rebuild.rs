...
mod tests {
    ...
    /// Three receipt days, each after the first walking back into the dates
    /// before it: day 10 live; day 11 live plus a walk into 10; day 13 a walk
    /// into 10 and 11. Every venue date is fed by more than one receipt day.
    fn three_walked_days() -> (tempfile::TempDir, Hyperliquid) {
        let dir = tempfile::tempdir().unwrap();
        let hl = adapter();
        let mut archive = Archive::open(dir.path().join("archive"));
        let venue = |day: i64, n: i64| day * DAY / 1_000 + n * 1_000;
        let frames = [
            (10 * DAY + 1, venue(10, 1), 1),
            (11 * DAY + 1, venue(11, 1), 10),
            (11 * DAY + 2, venue(10, 2), 11),
            (13 * DAY + 1, venue(10, 3), 20),
            (13 * DAY + 2, venue(11, 2), 21),
        ];
        for (recv, millis, seq) in frames {
            let mut payload = hl.classify(&bbo("BTC", millis), recv);
            payload.seq = seq;
    ...
    /// Every segment under a tape, by path relative to it, with its bytes.
    fn tape_contents(tape: &Path) -> Vec<(String, Vec<u8>)> {
    ...
    #[test]
    fn any_order_of_day_rebuilds_converges_on_the_full_rebuild() {
        // The property the defect broke: before, each per-day replacement
        // deleted what the other days wrote, so no order came out right.
        let (dir, hl) = three_walked_days();
        let root = dir.path().join("archive");
        let full = dir.path().join("full");
        rebuild(&root, &full, &hl, None, i64::MIN, i64::MAX).unwrap();
        let expected = tape_contents(&full);
        ...
        for (n, order) in orders.iter().enumerate() {
            // From an empty tape, and from one a full rebuild already wrote —
            // the nightly flow's case.
            for seeded in [false, true] {
                let tape = dir.path().join(format!("tape-{n}-{seeded}"));
                if seeded {
                    rebuild(&root, &tape, &hl, None, i64::MIN, i64::MAX).unwrap();
                }
                for &i in order {
                ...
                assert_eq!(
                    tape_contents(&tape),
                    expected,
                    "days rebuilt in order {:?} (seeded: {seeded}) differ from one full rebuild",
                    order.map(|i| days[i])
                );
            }
        }
